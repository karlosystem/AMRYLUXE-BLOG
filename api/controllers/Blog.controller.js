...
export const addBlog = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Blog agregado con exito.",
    });
  } catch (error) {
    next(handleError(500, error.message));
  }
     ...
     const { blogid } = req.params
        ...
        if (req.file) {
            ...
            const uploadResult = await cloudinary.uploader
                .upload(
                    req.file.path,
                    { folder: 'amryluxe-blog-mern', resource_type: 'auto' }
                )
                .catch((error) => {
                    next(handleError(500, error.message))
                });
...
export const getAllBlogs = async (req, res, next) => {
    try {
        // 1. Buscamos los blogs e indicamos a Mongoose que "infle" el ID de author y category con sus datos reales
        const blogs = await Blog.find()
            .populate('author', 'name avatar role') // 👈 ¡ESTA LÍNEA ES LA CLAVE! Trae nombre, foto y rol
            .populate('category', 'name slug')
            .sort({ createdAt: -1 }); // Ordena por los más recientes

        // 2. Enviamos la respuesta al Frontend
        res.status(200).json({ 
            success: true, 
            blogs // o 'blog', según cómo lo tengas mapeado en tu respuesta actual
        });
